...
function nav(depth=1, active="") {
  ...
  const links=[["","홈"],["rp/","PV 계산기"],["rp-value/","포인트 효율"],["dilution/","희석 계산기"],["cost/","원가 계산기"],["products/","제품 정보"],["insights/","데이터 분석"],["guide/","이용안내"],["faq/","FAQ"]];
...
write("insights/rp-scenarios/index.html",page({slug:"insights/rp-scenarios/",title:"20–100 PV 실제 조합 시나리오",description:"현재 PV 계산기와 같은 우선 규칙으로 20, 40, 60, 80, 100 PV의 실제 제품 조합을 비교합니다.",body:`<p class="breadcrumb"><a href="../">데이터 분석</a> / PV 시나리오</p><section class="card"><span class="eyebrow">계산기 로직 재현</span><h1>20–100 PV 실제 조합 시나리오</h1><p class="guide-lead">같은 PV라도 남는 PV를 먼저 줄인 뒤 제품 수 또는 결제금액을 비교하면 선택되는 조합이 달라질 수 있습니다.</p><p class="small">${dataDate} 제품 데이터 · 같은 제품 중복 없음(제품별 최대수량 1) · 포함/제외 제품 없음</p><div class="notice-box">표시된 결과는 설정 조건에서 계산기가 만든 조합 사례이며 구매 추천 목록이 아닙니다. 프로모션과 실시간 재고는 자동 반영되지 않으므로 실제 주문 시 Young Living 공식 주문 화면을 최종 확인하세요.</div></section><section class="card"><h2>실제 계산 결과</h2><div class="table-scroll"><table class="analysis-table scenario-table"><thead><tr><th>입력 PV</th><th>우선 기준</th><th>사용 PV</th><th>남는 PV</th><th>제품 수</th><th>총 회원가</th><th>제품 조합</th></tr></thead><tbody>${scenarioRows()}</tbody></table></div></section><section class="card"><h2>왜 결과가 달라질까요?</h2><p>현재 계산기는 먼저 목표를 넘지 않으면서 남는 PV가 가장 작은 조합을 찾습니다. 남는 PV가 같은 후보가 여럿일 때 선택한 우선 기준으로 제품 수 또는 결제금액을 비교합니다. 따라서 우선 기준은 남는 PV 최소화보다 뒤에 적용됩니다.</p><p>등록 제품의 PV 단위 조합만 사용할 수 있으므로 목표와 정확히 일치하는 합이 없으면 남는 PV가 0이 아닐 수 있습니다. 제품 수 최소는 주문 항목을 줄이는 데, 결제금액 낮은 순은 같은 PV 근처에서 회원가 합계를 낮추는 데 초점을 둡니다.</p><p>필수 제품을 포함하면 그 제품의 PV×수량을 먼저 차감한 뒤 나머지를 계산하므로 전체 결과가 달라집니다.</p><a class="btn secondary" href="../../rp/">PV 계산기에서 직접 조건 바꾸기 →</a></section>`}));
...
for(const file of detailFiles){let html=fs.readFileSync(file,"utf8");const id=Number((html.match(/data-product-id="(\d+)"/)||[])[1]);const p=products.find(x=>x.id===id);if(!p)continue;html=html.replace(/(<strong data-product-price>)[^<]*(<\/strong>)/,`$1${won(p.price)}$2`).replace(/(<strong data-product-rp>)[^<]*(<\/strong>)/,`$1${rp(p.rp).replace(/\.00$/,"")} PV$2`);fs.writeFileSync(file,html,"utf8");}
...
for(const file of allHtml){
  ...
  html=html.replace(/(<a(?: class="active")? href="[^"]*rp\/">PV 계산기<\/a>)(<a(?: class="active")? href="[^"]*cost\/">원가 계산기<\/a>)(<a(?: class="active")? href="[^"]*dilution\/">희석 계산기<\/a>)(<a(?: class="active")? href="[^"]*rp-value\/">포인트 효율<\/a>)/, "$1$4$3$2");
...
if(!home.includes('id="data-analysis-intro"')) home=home.replace('<section class="tips-card">',`<section class="card" id="data-analysis-intro"><span class="badge">직접 계산한 콘텐츠</span><h2>YL Toolkit 데이터 분석</h2><p>100개 제품 데이터를 이용한 포인트 효율 분포, PV별 실제 조합, 포인트 활용 사례 등을 확인할 수 있습니다.</p><a class="btn secondary" href="./insights/">분석실 보기 →</a></section>\n\n<section class="tips-card">`);
home=home.replace('<b>v4.8.0</b>',`<b>v${version}</b>`).replace('<ul>\n      <li>제품별 1PV당 제품가치를 계산하는 포인트 구매 효율 비교기 추가</li>',`<ul>\n      <li>100개 제품 데이터 기반 포인트 효율 분포와 카테고리 분석 추가</li>\n      <li>20·40·60·80·100 PV 실제 조합 및 포인트 활용 사례 추가</li>\n      <li>DIY 원가 사례와 데이터·계산 기준 페이지 추가</li>\n      <li>제품 상세 초기 HTML 가격·PV 자동 주입 구조 추가</li>`);
...
about=about.replace(/<h2>운영 목적<\/h2><p>.*?<\/p><p>/s,'<h2>운영 목적</h2><p>YL Toolkit은 실제 제품 구매와 DIY 준비 과정에서 PV 조합, 포인트 효율, 희석량과 원가를 반복 계산해야 하는 불편을 줄이기 위해 만든 개인 제작 무료 도구입니다. 프로그래밍 지식이 없어도 휴대전화에서 쉽게 사용할 수 있도록 모바일 중심으로 제작합니다.</p><p>');
if(!about.includes('<h3>포인트 구매 효율 비교기</h3>')) about=about.replace('<h2>데이터 관리 원칙</h2>','<h3>포인트 구매 효율 비교기</h3><p>회원가와 PV로 1PV당 제품가치를 계산해 제품 사이의 금액 대응 차이를 비교합니다.</p><h3>제품 정보센터</h3><p>등록된 8개 에센셜오일의 공식 사실정보와 YL Toolkit 편집 설명, 가격·PV를 구분해 제공합니다.</p><h3>데이터 분석 콘텐츠</h3><p>100개 제품의 실제 데이터와 계산기 공식을 바탕으로 분포, PV 조합과 원가 사례를 분석합니다.</p>\n<h2>데이터 관리 원칙</h2>');
